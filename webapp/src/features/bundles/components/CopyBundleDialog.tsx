...
interface CopyBundleDialogProps {
  open: boolean;
  sourceBundleName: string | null;
  sourceBundleSource: 'user' | 'system' | null;
  onClose: () => void;
...
export function CopyBundleDialog({
  open,
  sourceBundleName,
  sourceBundleSource,
  onClose,
  ...
  if (!sourceBundleName) return null;

  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Copy Bundle</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-sm">{sourceBundleName}</span>
              <span
                className={`text-xs px-1.5 py-0.5 rounded ${
                  sourceBundleSource === 'user'
                    ? 'bg-green-100 text-green-700'
                    : 'bg-gray-100 text-gray-600'
                }`}
              >
                {sourceBundleSource === 'user' ? 'User' : 'System'}
              </span>
            </div>
          </div>
