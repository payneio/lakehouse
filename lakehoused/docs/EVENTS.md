...
The lakehoused event system uses a single file (`events.jsonl`) as the source of truth for all session activity. This file is written by amplifier_core's hooks-logging module and read on-demand for trace visualization.
...
```python
from lakehoused.services.trace_aggregator import aggregate_events_to_turns
