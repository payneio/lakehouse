...
```
1. Bundle directory
   └─ Located at: ~/.lakehoused/bundles/{collection}/{bundle}/

2. Bundle → Mount Plan (dict with all configurations)
   └─ Generated by: LakehouseBundleManager
   └─ Saved to: .lakehoused/state/sessions/{id}/mount_plan.json
...
```
~/.lakehoused/bundles/foundation/base/
├── bundle.yaml
├── agents/
│   └── main-agent.md
└── context/
...
- Bundle not found → Check: `~/.lakehoused/bundles/`
...
This documentation follows lakehoused's implementation philosophy:
