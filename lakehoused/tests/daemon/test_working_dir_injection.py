...
def test_inject_runtime_config_working_dir(tmp_path: Path) -> None:
    ...
    from lakehoused.routers.sessions import _inject_runtime_config
    ...
    _inject_runtime_config(mount_plan, "session_123", absolute_project_path)
...
def test_inject_runtime_config_session_log_template_with_module_key(tmp_path: Path) -> None:
    ...
    with patch("lakehoused.routers.sessions.get_state_dir") as mock_get_state_dir:
        mock_get_state_dir.return_value = tmp_path / ".lakehoused" / "state"
        ...
        expected_path = str(tmp_path / ".lakehoused" / "state" / "sessions" / "{session_id}" / "events.jsonl")
...
def test_inject_runtime_config_session_log_template_singular_hook_logging(tmp_path: Path) -> None:
    """Test _inject_runtime_config injects session_log_template for hook-logging (singular).
