...
class TestApiKeyEnvOverride:
    ...
    def test_env_provides_key_without_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-from-env")
        secrets = load_secrets(secrets_path=tmp_path / "missing.yaml")
        assert secrets.api_keys["provider-anthropic"] == "sk-from-env"

    def test_env_overrides_file_key(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        secrets_path = tmp_path / "secrets.yaml"
    ...
    def test_file_key_kept_when_env_unset(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
