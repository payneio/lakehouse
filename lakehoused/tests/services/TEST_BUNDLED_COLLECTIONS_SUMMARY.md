...
Comprehensive test suite for the `bundled:lakehoused.data.collections.{name}` collection mechanism using `importlib.resources`.
...
# Run with coverage
uv run pytest tests/services/test_bundled_collections.py --cov=lakehoused.services.collection_service
